...
export const loadUrlToDataUrlClient = async (imageUrl: string) => {
  // Check cache first
...
export const getCachedDataUrl = async (data: string, fileName: string, mimeType: string) => {
  console.log("getCachedDataUrl");
  try {
    const d = decodeBase64(data);
...
export const createDataUrlFromLink = (imageUrl: string, fileName: string) => {
  return new Promise(async (res, rej) => {
    try {
      ...
      reader.onloadend = () => {
        ...
        res([undefined, { dataUrl, base64, mimeType: blob.type }]);
      };
      // const mimeType = await detectFileType(blob);
  ...
  }) as Promise<[Error, undefined] | [undefined, { dataUrl: string; base64: string; mimeType: string }]>;
};

export const getThumbNailDataUrl = async (url: string) => {
  try {
    // Fetch image
    ...
    console.log({ blob_type: blob.type, blobBuffer, size: blob.size });
    return [undefined, dataUrl] as const;
  } catch (err) {
    return [new Error(`Failed to load image: ${err instanceof Error ? err.message : "Unknown error"}`), undefined] as const;
  }
